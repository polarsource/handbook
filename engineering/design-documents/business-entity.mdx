...
1. **Polar Developer Experience (🟡 2)**: Type discrimination handled by SQLAlchemy polymorphism
...
def add_team_member_wrong(individual: IndividualCustomer, member: IndividualCustomer):
    individual.members.append(Member(...))  # ❌ mypy error: "IndividualCustomer" has no attribute "members"
```
